...
def fetch_poster(movie_title, TMDB_API_KEY):
    try:
        ...
        if search_response.status_code == 200:
            ...
            # Loop through search results to find exact match
            for result in search_data['results']:
                if result['title'].lower() == movie_title.lower():
                    movie_id = result['id']
                    
                    # Step 2: Fetch the movie details using the movie ID
            ...
            return "https://via.placeholder.com/500x750?text=No+Movie+Found"
        
        else:
            return "https://via.placeholder.com/500x750?text=Error+Searching+Movie"
    ...
    for row in range(num_rows):
                        ...
                        for col in range(num_cols):
                            ...
                            if index < len(num_movies):
                                ...
                                if poster_url == "https://via.placeholder.com/500x750?text=No+Movie+Found":
                                    continue
